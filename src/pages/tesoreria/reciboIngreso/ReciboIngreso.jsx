...
const ReciboIngreso = () => {
  ...
  useEffect(() => {
    GetPermisos("ReciboIngreso", setPermisos);
  }, []);
  //#endregion
  ...
  const Listar = async (filtro = "", pagina = 1) => {
    const result = await ApiMasy.get(
      `api/Tesoreria/ReciboIngreso/Listar?Pagina=${pagina}${filtro}`
    );
    setDatos(result.data.data.data);
    setTotal(result.data.data.total);
  };
  const GetPorId = async (id) => {
    const result = await ApiMasy.get(`api/Tesoreria/ReciboIngreso/${id}`);
    ...
    if (click) {
      ...
      switch (accion) {
        ...
        case 2: {
          ...
          if (valor) {
            await Delete("Tesoreria/ReciboIngreso", value, setListar);
          }
          break;
        }
        case 3: {
        ...
        case 5: {
          let row = document
            .querySelector("#tablaReciboIngreso")
            .querySelector("tr.selected-row");
          if (row != null) {
            let id = row.children[0].innerHTML;
            let model = await Imprimir("Tesoreria/ReciboIngreso", id);
            if (model != null) {
              setObjeto(model);
      ...
      {visible ? (
        <>
          <div className="h-full px-2 ">
            <h2 className={G.TituloH2}>Recibo Ingreso</h2>
